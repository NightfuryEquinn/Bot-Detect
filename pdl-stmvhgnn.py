...
# 1. Get Baseline AUC
with torch.no_grad():
    base_logits = last_model(x, edge_index_auc, edge_index_ip, edge_index_dev, weight_auc, weight_ip, weight_dev).squeeze()
...
for j, feat_name in enumerate(feature_names):
    ...
    with torch.no_grad():
        perm_logits = last_model(x_perm, edge_index_auc, edge_index_ip, edge_index_dev, weight_auc, weight_ip, weight_dev).squeeze()
...
colors = [PALETTE.get("stmvhgnn", "#D35400") if val > 0 else PALETTE.get("neutral", "#6C757D") for val in top_perm["auc_drop"]]
